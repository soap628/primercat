...
const COPY = {
  zh: {
    ...
    heroMetric: "V1 安全目录",
    heroValue: "11 种常见试剂",
    heroBody: "强调高风险实验室试剂，优先展示会改变操作方式的急性毒性、腐蚀、易燃和长期健康危害。",
    ...
    searchLabel: "搜索试剂",
    searchPlaceholder: "名称、英文名、CAS、化学式，例如：甲醇 / 67-56-1 / MeOH",
    all: "全部",
    acute: "急性毒性",
    corrosive: "腐蚀",
    flammable: "易燃",
  ...
  en: {
    ...
    heroMetric: "V1 safety catalog",
    heroValue: "11 common reagents",
    heroBody: "The catalog prioritizes laboratory chemicals whose acute toxicity, corrosivity, flammability, or chronic hazards materially change how work should be performed.",
    ...
    searchPlaceholder: "Name, CAS, or formula — e.g. methanol / 67-56-1 / MeOH",
    all: "All",
    ...
    reviewed: "Content reviewed: 2026-08-29",
...
function ChemicalCard({ record, zh }: { record: ChemicalSafetyRecord; zh: boolean }) {
  ...
  const pubchemUrl = `https://pubchem.ncbi.nlm.nih.gov/compound/${record.cid}`;
  return (
    <details className="chem-card" id={record.id}>
      ...
      <div className="chem-card-detail">
        ...
        <a className="chem-source-link" href={pubchemUrl} target="_blank" rel="noreferrer">{copy.source} · CID {record.cid} ↗</a>
      </div>
    </details>
  );
}
...
export default function ChemicalSafetyPage({
  ...
  const filters: SafetyFilter[] = ["all", "acute", "corrosive", "flammable", "chronic", "irritant"];
