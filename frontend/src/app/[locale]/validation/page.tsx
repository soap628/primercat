...
const COPY = {
  zh: {
    ...
    abstract: "PrimerCat 不为所有结果提供一个统一的“准确率”。我们把输入质量、算法输出、数据库覆盖、启发式假设和实验确认分别说明，帮助研究者判断当前证据能支持什么结论。",
    meta: ["证据框架 1.6", "更新：2026-09-03", "研究用途"], asideTitle: "核心结论",
    aside: "PrimerCat 可以精确计算数值，但不能据此保证生物学结论。研究者不应把“未见命中”解释为无风险，也不应把排名第一解释为实验必然成功。",
    ...
    claimsTitle: "各工具可支持的结论", claimsLead: "PrimerCat 在下表中分别说明每个模块能支持的判断，以及研究者不能从结果继续推导的结论。",
    ...
    statusTitle: "状态标签如何解释", statusLead: "PrimerCat 的状态标签只总结当前筛查；研究者不能把“没有发现证据”直接解释为“安全”或“特异”。",
    statusRows: [
    ...
    experimentalTitle: "建议的实验验证路径", experimentalLead: "我们只提供最低限度的验证方向，而不是统一 SOP。研究者应根据自己的实验体系预先定义接受标准。",
    validationGroups: [
    ...
    reproTitle: "复现与变更因素", reproLead: "研究者若要让他人重建一次结果，应同时保存计算上下文，而不只是复制一条候选序列。",
    ...
    productionTitle: "当前运行依据与核验记录", productionLead: "为使 PrimerCat 的结果可追溯，我们在这里固定公开当前服务采用的参考版本、索引规模、软件质量基线和线上抽查结果。研究者应在标注的数据范围、规则与检查日期内解释每项结论。",
    ...
    benchmarkTitle: "固定参考计算覆盖审计 v0.6", benchmarkLead: "我们从固定版小鼠 RefSeq RNA 按公开、确定性的散列规则选取 200 个不同基因，每个基因保留一个 NM_ 转录本，再由 PrimerCat 重新生成并筛查候选。这项审计扩大了流程覆盖面，但不是随机抽样、外部准确率测试或湿实验验证。",
    ...
    currentTitle: "当前验证状态", currentLead: "我们主动公开 PrimerCat 尚未建立的证据，避免研究者把软件可用性误解为学术验证。",
    currentItems: [
      ["我们已公开逐候选计算记录", "v0.6 从固定 RefSeq RNA 中确定性选取 200 个不同基因；我们同时公开抽样规则、参考文件校验值、运行参数、逐候选筛查记录和代码校验值。"],
      ["PrimerCat 尚无临床验证", "PrimerCat 仅用于研究设计，不用于诊断、治疗决策、临床报告或监管提交。"],
    ...
    ctaTitle: "把“可信”变成可检查的记录", ctaBody: "研究者应先确认输入与筛查后端，再阅读命中详情，最后在实际实验体系中完成验证。", methods: "查看完整方法", primer: "开始 qPCR 设计",
  },
  en: {
    eyebrow: "PRIMERCAT · EVIDENCE & VALIDATION NOTE", title: "How to assess confidence in a result", abstractLabel: "Abstract",
    abstract: "PrimerCat does not assign one universal accuracy value to every result. We describe input quality, algorithmic output, database coverage, heuristic assumptions, and experimental confirmation separately so researchers can judge what each layer of evidence supports.",
    meta: ["Evidence framework 1.6", "Updated 2026-09-03", "Research use only"], asideTitle: "Central conclusion", aside: "PrimerCat can calculate a value precisely without guaranteeing the biological conclusion. Researchers should not read no database hit as no risk, or a first-ranked candidate as guaranteed to work.",
    ...
    evidenceRows: [["C1", "Deterministic computation", "Sequence length, GC%, coordinates, solution arithmetic, Primer3 fields", "PrimerCat should reproduce the value under the same input, parameters, and software version; input error and model assumptions still apply."], ["D1", "Database-dependent screen", "RefSeq template, BLAST/Bowtie2 hits, accessions", "PrimerCat's statement is valid only for the stated database/assembly, query date, thresholds, and hit cap."], ["H1", "Heuristic rank", "qPCR composite score, gRNA activity score, Low/Medium/High labels", "Researchers may use it to prioritise candidates, but not as a probability or a value comparable across tools or experiments."], ["R1", "Curated reference", "Protocols, recipe notes, chemical-hazard summaries", "Researchers may use it for preparation and review, but not instead of primary literature, product instructions, SDS, SOP, or risk assessment."], ["X0", "Experimental confirmation", "Efficiency, single product, measured editing, measured off-targets, sample suitability", "PrimerCat does not produce this evidence; researchers must establish it in their own experimental system."]],
    ...
    claimsTitle: "Claims supported by each tool", claimsLead: "PrimerCat states what each module can support and what researchers cannot infer from the result.",
    ...
    statusTitle: "How to read status labels", statusLead: "A PrimerCat status summarises the current screen. Researchers must not convert absence of evidence into a claim of safety or specificity.",
    ...
    experimentalTitle: "Recommended experimental validation", experimentalLead: "We provide minimum validation directions, not a universal SOP. Researchers should pre-specify acceptance criteria for their actual experimental system.",
    ...
    reproTitle: "Reproducibility and change factors", reproLead: "To let another researcher reconstruct a run, researchers should preserve the computational context rather than only a copied candidate sequence.",
    ...
    productionTitle: "Current operating basis and verification record", productionLead: "To keep PrimerCat results traceable, we publish the reference versions, index scale, software quality baseline, and production spot checks used by the current service. Researchers should interpret each conclusion within the stated data scope, rules, and check date.",
    ...
    ctaTitle: "Turn confidence into an inspectable record", ctaBody: "Researchers should confirm the input and screening backend, inspect hit details, and then validate in the actual experimental system.", methods: "Read full methods", primer: "Start qPCR design",
  },
} as const;
